...
public class Rates_HO4 extends TestBase {
	...
	ExtentHtmlReporter reporter = new ExtentHtmlReporter("T:\\Automations\\ExtentReports\\IAQ\\Testing\\Rates\\TN\\Rates_HO4\\Rates_HO4-" + GetDateTime() + ".html");

	@DataProvider(name="Rates_HO4")
	public String[][] GetRates_HO4DataCall() {
		String[][] data = GetData("IAQPoldata", "Rates_HO4", "Testing", "TN");
		return data;
	}
		...
		try {
			...
			if(raterprem - iaqprem > 1 || iaqprem-raterprem > 1) {
				...
				if(!allotherperils.equalsIgnoreCase(PRP2.ReturnAllOtherPerilsDeductible())) {
						Log("All Other Perils: " + allotherperils + "-" + PRP2.ReturnAllOtherPerilsDeductible());
				}
			...
			e.printStackTrace();
			GetFailScreenShot(casenum + "-TN_HO4_Error");
