package Automation.IAQ.Testing.Rates.IN;
...
public class Rates_HO3 extends TestBase {
	...
	ExtentHtmlReporter reporter = new ExtentHtmlReporter("T:\\Automations\\ExtentReports\\IAQ\\Testing\\Rates\\IN\\Rates_HO3\\Rates_HO3-" + GetDateTime() + ".html");

	@DataProvider(name="Rates_HO3")
	public String[][] GetRates_HO3DataCall() {
		String[][] data = GetData("IAQPoldata", "Rates_HO3", "Testing", "IN");
		return data;
	}
		...
		try {
			...
			switch(basement.toUpperCase()) {
				case "NONE": case "NO": case "N":		if(substruct.toUpperCase().equals("B") || substruct.toUpperCase().equals("BASEMENT")) {
														...
														P.ForceSelectSubstructure(substruct);
														break;
			...
			CBP.SelectWindHailDeductible(windhaildeduct);
				Pause(3000);
			CBP.SelectEarthquakeCoverage(eqcovg);
			if(eqcovg.toUpperCase().equals("Y") || eqcovg.toUpperCase().equals("YES")) {
				Pause(8000);
			}
			else {
				Pause(3000);
			}
			if(driver.getPageSource().contains("Earthquake Deductible")) {
				CBP.SelectEarthquakeDeductible(eqdeduct);
					Pause(3000);
			}
			if(driver.getPageSource().contains("Earthquake Secured Water Heater")) {
				CBP.SelectEarthquakeSecuredWaterHeater(securewaterheater);
					Pause(3000);
			}
			...
			if(hspackage.toUpperCase().equals("N") || (hspackage.toUpperCase().equals("NO"))) {
				...
				Pause(3000);
			if(driver.getPageSource().contains("Mine Subsidence Coverage A")) {
				CBP.SelectMineSubsidenceA(minesub);
					Pause(3000);
			}
			if(driver.getPageSource().contains("Mine Subsidence Coverage B")) {
				CBP.SelectMineSubsidenceB(minesubcovgb);
					Pause(3000);
			}
			...
			if(raterprem - iaqprem > 1 || iaqprem-raterprem > 1) {
				...
				if(!eqcovg.equalsIgnoreCase(PRP2.ReturnEarthquakeCoverage())) {
						Log("Earthquake Coverage: " + eqcovg + "-" + PRP2.ReturnEarthquakeCoverage());
				}
				if(!eqdeduct.equalsIgnoreCase(PRP2.ReturnEarthquakeDeductible())) {
						Log("Earthquake Deductible: " + eqdeduct + "-" + PRP2.ReturnEarthquakeDeductible());
				}
				if(!substruct.equalsIgnoreCase(PRP2.ReturnEarthquakeFoundationType())) {
						Log("Earthquake Foundation Type: " + substruct + "-" + PRP2.ReturnEarthquakeFoundationType());
				}
				if(!securewaterheater.equalsIgnoreCase(PRP2.ReturnEarthquakeSecuredWaterHeater())) {
						Log("Earthquake Secured Water Heater: " + securewaterheater + "-" + PRP2.ReturnEarthquakeSecuredWaterHeater());
				}
				...
				if(!minesub.equalsIgnoreCase(PRP2.ReturnMineSubsidenceCoverage())) {
						Log("Mine Subsidence Coverage A: " + minesub + "-" + PRP2.ReturnMineSubsidenceCoverage());
				}
				...
				if(!minesubcovgb.equalsIgnoreCase(PRP2.ReturnMineSubsidenceCoverageB())) {
						Log("Mine Subsidence Coverage B: " + minesubcovgb + "-" + PRP2.ReturnMineSubsidenceCoverageB());
				}
