...
public class Rates_HO6 extends TestBase {
	...
	ExtentHtmlReporter reporter = new ExtentHtmlReporter("T:\\Automations\\ExtentReports\\IAQ\\Testing\\Rates\\KS\\Rates_HO6\\Rates_HO6-" + GetDateTime() + ".html");
	
	@DataProvider(name="Rates_HO6")
	public String[][] GetRates_HO6DataCall() {
		String[][] data = GetData("IAQPoldata", "Rates_HO6", "Testing", "KS");
		return data;
	}
	...
	public void RunRates_HO6(String casenum, String state, String planid, String pgragentid, String asiagentid, String fname, String lname,
		...
		try {
			...
			P.TypeDwellingCoverage(covga);
				Pause(1000);
			if(newpurch.toUpperCase().equals("Y") || newpurch.toUpperCase().equals("YES") || newpurch.toUpperCase().equals("1")) {
				P.ForceTypePurchasePrice("200000");
					Pause(1000);
			}
			...
			if(raterprem - iaqprem > 1 || iaqprem-raterprem > 1) {
				...
				if(!PRP1.ReturnNumofStories().equals("Condo 1 Story")) {
						Log("Number of Stories: Condo 1 Story-" + PRP1.ReturnLocationZip());
				}
			...
			e.printStackTrace();
			GetFailScreenShot(casenum + "-KS_HO6_Error");
