package Automation.IAQ.Testing.Rates.IL;
...
public class Rates_HO3 extends TestBase {
	...
	ExtentHtmlReporter reporter = new ExtentHtmlReporter("T:\\Automations\\ExtentReports\\IAQ\\Testing\\Rates\\IL\\Rates_HO3\\Rates_HO3-" + GetDateTime() + ".html");

	@DataProvider(name="Rates_HO3")
	public String[][] GetRates_HO3DataCall() {
		String[][] data = GetData("IAQPoldata", "Rates_HO3", "Testing", "IL");
		return data;
	}
	...
	public void RunRates_HO3(String casenum, String state, String planid, String pgragentid, String asiagentid, String fname, String lname,
		...
		try {
			...
			if(hspackage.toUpperCase().equals("N") || (hspackage.toUpperCase().equals("NO"))) {
				...
				Pause(3000);
			CBP.SelectUndamagedSidingRoofing(matchundmgsiding);
				Pause(3000);
			if(driver.getPageSource().contains("Mine Subsidence Coverage")) {
			...
			if(raterprem - iaqprem > 1 || iaqprem-raterprem > 1) {
				...
				if(!minesub.equalsIgnoreCase(PRP2.ReturnMineSubsidenceCoverage())) {
						Log("Mine Subsidence Coverage: " + minesub + "-" + PRP2.ReturnMineSubsidenceCoverage());
				}
			...
			e.printStackTrace();
			GetFailScreenShot(casenum + "-IL_HO3_Error");
