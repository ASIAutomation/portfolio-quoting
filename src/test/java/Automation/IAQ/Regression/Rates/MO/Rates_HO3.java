package Automation.IAQ.Regression.Rates.MO;
...
public class Rates_HO3 extends TestBase {
	...
	ExtentHtmlReporter reporter = new ExtentHtmlReporter("T:\\Automations\\ExtentReports\\IAQ\\Regression\\Rates\\MO\\Rates_HO3\\Rates_HO3-" + GetDateTime() + ".html");

	@DataProvider(name="Rates_HO3")
	public String[][] GetRates_HO3DataCall() {
		String[][] data = GetData("IAQPoldata", "Rates_HO3", "Regression", "MO");
		return data;
	}
		...
		try {
			...
			if(driver.getPageSource().contains("Home Update")) {
				if(hud.toUpperCase().equals("N/A")) {
					...
					P.ForceSelectHomeUpdate(hud);
					if(hud.toUpperCase().equals("Y") || hud.toUpperCase().equals("YES")) {
						P.ForceTypeLatestPermitDate(latestpermit);
					}
				}
			}
			...
			if(!premium.equals(CBP.ReturnPremium("N"))) {
				...
				GetFailScreenShot(casenum + "-MO_HO3_Error");
