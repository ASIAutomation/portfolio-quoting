package Automation.IAQ.Regression.Rates.GA;
...
public class Rates_HO3 extends TestBase {
	...
	ExtentHtmlReporter reporter = new ExtentHtmlReporter("T:\\Automations\\ExtentReports\\IAQ\\Regression\\Rates\\GA\\Rates_HO3\\Rates_HO3-" + GetDateTime() + ".html");

	@DataProvider(name="Rates_HO3")
	public String[][] GetRates_HO3DataCall() {
		String[][] data = GetData("IAQPoldata", "Rates_HO3", "Regression", "GA");
		return data;
	}
	...
	public void RunRates_HO3(String casenum, String state, String planid, String pgragentid, String asiagentid, String fname, String lname,
		...
		try {
			...
			P.SelectResidenceType("Single Family");
				Pause(2500);
			P.SelectExcludeWind(excludewind);
				Pause(2500);
			...
			if(nshl.toUpperCase().equals("Y") || nshl.toUpperCase().equals("YES")) {
				P.SelectExteriorWalls("Aluminum Siding");
			}
			else {
			...
			AD.SelectWindowandOtherOpeningProtection(windowandotheropeningprotection);
			
			int numyearhomebuilt = Integer.parseInt(yearhomebuilt);
			System.out.println("num year homebuilt =  " + numyearhomebuilt);
			if (numyearhomebuilt > 2007) 
			{
				AD.SelectBuildingCode(buildingcode);
			}
			...
			if(driver.getPageSource().contains("Non Structural Hail Loss Limitation")) {
				CBP.CheckNonStructuralHailLossLimitation(nshl);
			}
			Pause(3000);
			...
			CBP.CheckIncreasedReplacementCostonDwelling(increpcost);
			...
			if(!premium.equals(CBP.ReturnPremium("N"))) {
				...
				if(!excludewind.equalsIgnoreCase(PRP1.ReturnExcludeWind())) {
						Log("Exclude Wind:" + excludewind + "-" + PRP1.ReturnExcludeWind());
				}
				if(accbuilder.toUpperCase().equals("Y") || accbuilder.toUpperCase().equals("YES")) {
				...
				if(!buildingcode.equalsIgnoreCase(PRP2.ReturnBuildingCode())) {
						Log("Building Code: " + buildingcode + "-" + PRP2.ReturnBuildingCode());
				}
				...
				if(!windowandotheropeningprotection.equalsIgnoreCase(PRP2.ReturnWindowandOtherOpeningProtection())) {
						Log("Window and Other Opening Protection: " + windowandotheropeningprotection + "-" + PRP2.ReturnWindowandOtherOpeningProtection());
				}
				...
				if(!nshl.equalsIgnoreCase(PRP2.ReturnNonStructuralHailLossLimitation())) {
						Log("Non Structural Hail Loss Limitation: " + nshl + "-" + PRP2.ReturnNonStructuralHailLossLimitation());
				}
				...
				e.printStackTrace();
				GetFailScreenShot(casenum + "-GA_HO3_Error");
