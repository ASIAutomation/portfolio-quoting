...
public class Rates_HO4 extends TestBase {
	...
	ExtentHtmlReporter reporter = new ExtentHtmlReporter("T:\\Automations\\ExtentReports\\IAQ\\Regression\\Rates\\TX\\Rates_HO4\\Rates_HO4-" + GetDateTime() + ".html");

	@DataProvider(name="Rates_HO4")
	public String[][] GetRates_HO4DataCall() {
		String[][] data = GetData("IAQPoldata", "Rates_HO4", "Regression", "TX");
		return data;
	}
		...
		try {
			...
			if(!premium.equals(CBP.ReturnPremium("N"))) {
				...
				if(!allotherperils.equalsIgnoreCase(PRP2.ReturnAllOtherPerilsDeductible())) {
						Log("Paid in Full Discount: " + allotherperils + "-" + PRP2.ReturnAllOtherPerilsDeductible());
				}
				...
				e.printStackTrace();
				GetFailScreenShot(casenum + "-TX_HO4_Error");
