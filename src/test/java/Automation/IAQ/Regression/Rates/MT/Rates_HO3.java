...
public class Rates_HO3 extends TestBase {
	...
	ExtentHtmlReporter reporter = new ExtentHtmlReporter("T:\\Automations\\ExtentReports\\IAQ\\Regression\\Rates\\MT\\Rates_HO3\\Rates_HO3-" + GetDateTime() + ".html");

	@DataProvider(name="Rates_HO3")
	public String[][] GetRates_HO3DataCall() {
		String[][] data = GetData("IAQPoldata", "Rates_HO3", "Regression", "MT");
		return data;
	}
	...
	public void RunRates_HO3(String casenum, String state, String planid, String pgragentid, String asiagentid, String fname, String lname,
		...
		try {
			...
			if(hspackage.toUpperCase().equals("N") || (hspackage.toUpperCase().equals("NO"))) {
				...
				Pause(3000);
			CBP.SelectHobbyFarming(hobbyfarming);
				Pause(3000);
			...
			if(!premium.equals(CBP.ReturnPremium("N"))) {
				...
				if(!hobbyfarming.equalsIgnoreCase(PRP2.ReturnHobbyFarmEndorsement())) {
						Log("Hobby Farm Endorsement: " + hobbyfarming + "-" + PRP2.ReturnHobbyFarmEndorsement());
				}
				...
				e.printStackTrace();
				GetFailScreenShot(casenum + "-MT_HO3_Error");
